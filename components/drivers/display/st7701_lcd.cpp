...
esp_lcd_dsi_bus_handle_t dsi_bus_handle = NULL;  // Store DSI bus handle for esp_lvgl_port
...
{   
    ...
    ESP_ERROR_CHECK(esp_lcd_new_dsi_bus(&bus_config, &dsi_bus_handle));

    ESP_LOGI(TAG, "Install MIPI DSI LCD control panel");
    // 我们使用DBI接口发送LCD命令和参数
    esp_lcd_dbi_io_config_t dbi_config = ST7701_PANEL_IO_DBI_CONFIG();

    ESP_ERROR_CHECK(esp_lcd_new_panel_io_dbi(dsi_bus_handle, &dbi_config, &io_handle));

    // 创建DPI面板配置 - 使用与工作示例相同的时序
    // refresh_rate = 34MHz / (480+12+42+42) / (800+2+8+166) = ~60Hz
    // CRITICAL: num_fbs = 2 for avoid_tearing!
    esp_lcd_dpi_panel_config_t dpi_config = {};
    dpi_config.dpi_clk_src = MIPI_DSI_DPI_CLK_SRC_DEFAULT;
    dpi_config.dpi_clock_freq_mhz = 34;
    dpi_config.virtual_channel = 0;
    dpi_config.pixel_format = MIPI_DPI_PX_FORMAT;
    dpi_config.num_fbs = 2;  // CRITICAL: 2 buffers for avoid_tearing!
...
void st7701_lcd::get_handle(bsp_lcd_handles_t *ret_handles)
{
    ...
    ret_handles->mipi_dsi_bus = dsi_bus_handle;  // Return DSI bus handle for esp_lvgl_port
